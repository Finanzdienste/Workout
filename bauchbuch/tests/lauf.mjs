/*
 * Alle Tests der Reihe nach.
 *
 *     node tests/lauf.mjs                # alles
 *     node tests/lauf.mjs rechnen muster # nur diese
 *
 * Nacheinander, nicht nebeneinander: Jeder Test startet einen echten Browser,
 * und mehrere davon gleichzeitig machen die Läufe unzuverlässig.
 *
...
// Die Ein-Datei-Fassung wird von zwei Tests geprüft. Fehlt sie, liegt das fast
// immer daran, dass sie nach einer Änderung nicht neu gebaut wurde – ein
// Hinweis ist hier nützlicher als sieben Zeilen "Datei nicht gefunden".
if (!existsSync(path.join(ROOT, 'dist', 'bauchbuch.html'))) {
  console.error('dist/bauchbuch.html fehlt – erst `python3 tools/build-single.py` laufen lassen.');
  process.exit(2);
}

// 8199 gewöhnlich, 8200 mit Haltbarkeit – siehe server.mjs.
const server = [await starte(8199, 0), await starte(8200, 600)];
